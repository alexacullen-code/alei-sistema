...
function calculateTuition(student, monthYear, paymentDate, baseAmount) {
  ...
  // Fechas límite para el mes de pago
  const limitDiscount = new Date(payYear, payMonth - 1, 10); // Día 10 = descuento
  const limitNormal = new Date(payYear, payMonth - 1, 15);   // Día 15 = normal
  
  // Determinar si está pagando el mes actual, anterior o siguiente
  ...
  // Si paga antes del día 10 del mes que corresponde → Descuento
  // Si paga entre 11-15 → Precio normal
  // Si paga después del 16 → Recargo
  
  if (monthDiff > 0) {
    // Pagando mes(es) futuro(s) → Descuento
    discountAmount = 150;
  } else if (monthDiff < 0) {
    // Pagando mes(es) pasado(s) → Recargo
    surchargeAmount = 150;
  } else {
    // Pagando el mes actual
    if (payDay <= 10) {
      discountAmount = 150;
    } else if (payDay >= 16) {
      surchargeAmount = 150;
...
async function getStudentFullProfile(req, res, pool) {
  ...
  const pendingTuition = await pool.query(
    `SELECT * FROM tuition_payments 
     WHERE student_id = $1 AND pending_amount > 0`,
    [id]
  );
  ...
  return res.json({
    ...
    pending_debts: {
      books: pendingBooks.rows,
      tuition: pendingTuition.rows
    }
  });
}
    ...
    if (existingResult.rows.length > 0) {
      ...
      result = await client.query(
        `UPDATE tuition_payments 
         SET amount_paid = $1, pending_amount = $2, status = $3, payment_date = $4
  ...
  try {
    ...
    const paymentResult = await client.query(
      `INSERT INTO book_payments 
       (assignment_id, amount, payment_method, comment, previous_balance, new_balance, created_by)
    ...
    await client.query(
      ...
      [newBalance, status, assignment_id]
    );
...
async function quickPayment(req, res, pool) {
  ...
  // Modo POS simplificado - detecta automáticamente el tipo de pago
  if (concept === 'book') {
    // Buscar libro pendiente
...
async function assignBook(req, res, pool) {
  ...
  try {
    ...
    // Crear asignación
    const assignmentResult = await client.query(
      `INSERT INTO book_assignments (student_id, book_id, total_price, pending_balance)
...
async function getWaitingList(req, res, pool, queryParams) {
  ...
  let sql = `SELECT w.*, l.name as level_name 
             FROM waiting_list w 
...
async function addToWaitingList(req, res, pool) {
  ...
  const result = await pool.query(
    `INSERT INTO waiting_list (year_id, level_id, student_name, parent_name, phone, email, notes)
     VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
...
async function getCashFlowReport(req, res, pool, queryParams) {
  ...
  const expensesResult = await pool.query(
    `SELECT COALESCE(SUM(total_amount), 0) as total
     FROM expenses
     WHERE year_id = $1 AND DATE_TRUNC('month', start_date) = $2::date`,
    [year_id, month + '-01']
  );
  ...
  return res.json({
    ...
    expenses: parseFloat(expensesResult.rows[0].total),
    balance: (parseFloat(tuitionIncome.rows[0].total) + parseFloat(bookIncome.rows[0].total)) - parseFloat(expensesResult.rows[0].total)
  });
}
...
async function backupEntity(req, res, pool) {
  ...
  res.setHeader('Content-Disposition', `attachment; filename="${entity}-backup.json"`);
...
async function getSystemStats(req, res, pool, queryParams) {
  ...
  stats.students = studentsResult.rows[0];
  ...
  stats.today_income = todayIncome.rows[0];
  ...
  stats.pending_debts = debtsResult.rows[0];
  ...
  stats.unread_alerts = parseInt(alertsResult.rows[0].total);
...
async function getEnrollmentTypes(req, res, pool, queryParams) {
  ...
  const result = await pool.query(
    'SELECT * FROM enrollment_types WHERE year_id = $1 ORDER BY base_price',
    [year_id]
  );
...
async function getLevels(req, res, pool, queryParams) {
  ...
  const result = await pool.query(
    'SELECT * FROM levels WHERE year_id = $1 ORDER BY name',
    [year_id]
  );
